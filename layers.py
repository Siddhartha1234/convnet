...
class Layers:
    def __init__(self):
        self.convc = Conv()
    def conv_layer(self, input_vector_volume, kernel_shape, num_kernels, stride, padding,
            non_linear_func):
        output_array = []
        for n in range(num_kernels):
            output_array.append(self.convc.conv(input_vector_volume,
                kernel_shape, stride, padding, non_linear_func))

        return np.array(output_array)

    def pool_layer(self, input_vector_volume, filter_size, stride, pooling_function):
        output_array = []
        for i in range(input_vector_volume.shape[0]):
            output_array.append(self.convc.pool(input_vector_volume[i],
                filter_size, stride, pooling_function))
        return np.array(output_array)

    def unravel_layer(self, input_vector_volume, output_num_nodes):
    ...
    def MLP_layer(self, input_vector, num_hidden_layers, hidden_size,
